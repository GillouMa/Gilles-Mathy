...
```{r}
ggplot(final_grade_df, aes(x = factor(group), y = final_grade)) +
...
```{r}
ggplot(final_grade_df, aes(x = factor(group), y = final_grade, fill = sex)) +
...
```{r}
trimester1 <- c(
  "Steam Engine Mechanics and Engineering",
...
trimester3 <- c(
  "Airship Piloting and Navigation",
  "Alchemy and Chemical Engineering",
  "Clockwork Automata and Robotics",
  "Fashion Design and Textile Innovation"
)

pass_df <- avg_grade_wide |>
  mutate(
    
    min_course_grade = apply(across(all_of(course_cols)), 1, min),

    avg_trimester1 = rowMeans(across(all_of(trimester1))),
    avg_trimester2 = rowMeans(across(all_of(trimester2))),
    avg_trimester3 = rowMeans(across(all_of(trimester3))),

  
    pass = min_course_grade >= 5 &
           avg_trimester1 >= 10 &
           avg_trimester2 >= 10 &
           avg_trimester3 >= 10
  ) |>
...
pass_df <- pass_df |>
  left_join(final_grade_df |> select(id, final_grade), by = "id") |>
...
```{r}
n_non_passing_final10 <- pass_df |>
  filter(pass == FALSE, final_grade >= 10) |>
  nrow()

```

The number of students who do not pass but still have a final grade of at least 10 is
`r n_non_passing_final10`.
...
pass_rate_group <- pass_df |>
  group_by(group) |>
  summarize(pass_rate = mean(pass) * 100)
...
course_failures_named <- course_failures |>
  left_join(course_names, by = "course_id") |>
  select(course_id, course_name, failures = n)

worst_course <- course_failures_named |>
  slice_max(failures, n = 1)

knitr::kable(worst_course)
```
...
ggplot(success_by_age, aes(x = factor(age), y = success_rate)) +
  geom_col(fill = "steelblue") +
...
There does not seem to be any particular effect of age on the success rate. The graph does clearly highly something like "Being older is associated with a higher success rate", maybe just very slightly. 
